...
public class HistoryManager {
    ...
    private static final int DEFAULT_MIN_DURATION = 15;
    private static final int DEFAULT_DEDUPE_WINDOW = 300;
    ...
    public boolean shouldSaveHistory(String trackId, int playDurationSeconds) {
        ...
        if (!isHistoryEnabled()) {
            Log.d(TAG, "History tracking is disabled in settings");
            return false;
        }

        // 2. Kiểm tra track ID hợp lệ
        if (trackId == null || trackId.trim().isEmpty()) {
            Log.w(TAG, "Invalid track ID, cannot save history");
            return false;
        }
        ...
        if (playDurationSeconds < minDuration) {
            Log.d(TAG, "Thời lượng phát (" + playDurationSeconds + "s) < tối thiểu (" + minDuration
                    + "s), skipping history");
            return false;
        }
        ...
        if (isRecentlyPlayed(trackId)) {
            Log.d(TAG, "Bản nhạc vừa được phát gần đây (trong cửa sổ dedupe), bỏ qua để tránh trùng lặp");
            return false;
        }

    Log.d(TAG, "Tất cả kiểm tra OK, sẽ lưu vào lịch sử");
        return true;
    }

    // Lưu lịch sử bằng auto-save API
    public void saveHistoryAutoSave(
        ...
        Log.d(TAG, "Calling API endpoint: POST /api/deezer/tracks/" + trackId + "/play?play_duration_seconds="
                + playDurationSeconds);

        deezerApi.playTrack(trackId, playDurationSeconds).enqueue(new Callback<PlayTrackResponse>() {
            @Override
            public void onResponse(@NonNull Call<PlayTrackResponse> call,
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Log.d(TAG, "History auto-saved successfully: " + track.title);
                    if (callback != null) {
                        callback.onSuccess();
                    }
                } else {
                    Log.e(TAG, "Tự động lưu lịch sử thất bại. HTTP " + response.code());
                    if (callback != null) {
    ...
    /**
     * Lưu lịch sử với DeezerApi (Manual method - fallback)
     */
    public void saveHistory(
        ...
        deezerApi.addHistoryRecord(request).enqueue(new Callback<Void>() {
            @Override
            public void onResponse(@NonNull Call<Void> call, @NonNull Response<Void> response) {
                if (response.isSuccessful()) {
                    ...
                    Log.e(TAG, "Lưu lịch sử thất bại. HTTP " + response.code());
                    if (callback != null) {
                        callback.onError(response.code(), "HTTP " + response.code());
                    }
                }
    ...
    Log.d(TAG, enabled ? "Theo dõi lịch sử: bật" : "Theo dõi lịch sử: tạm dừng");
    }
    ...
    public void setMinPlayDuration(int seconds) {
        ...
        Log.d(TAG, "Min play duration set to " + seconds + " seconds");
    }
    ...
    Log.d(TAG, "Cửa sổ dedupe được đặt: " + seconds + " giây");
    }
    ...
    private void cleanupOldEntries() {
        ...
        if (!toRemove.isEmpty()) {
            ...
            saveRecentTracks();
            Log.d(TAG, "Đã dọn " + toRemove.size() + " mục cũ");
        }
    }
    ...
    public void clearRecentTracks() {
        ...
        saveRecentTracks();
    Log.d(TAG, "Đã xóa tất cả bản nhạc gần đây");
    }
}
